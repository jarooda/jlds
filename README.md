...
The repo ships a tracked pre-commit hook in [`.githooks/`](.githooks/) that regenerates derived,
committed artifacts (currently the [`registry/css/all.css`](registry/css/all.css) bundle) and
...
The hook only touches committed artifacts — generated-but-gitignored output (docs `public/css`,
...
Want everything in one link? Use the bundle, which `@import`s `index.css` plus every component:

```html
<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/<org>/jlds@main/registry/css/all.css">
```

`css/all.css` is generated by `node registry/scripts/build-css-bundle.mjs` — re-run it whenever you add or remove a component stylesheet.
